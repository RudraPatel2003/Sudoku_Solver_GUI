...
class SudokuBoard:
    ...
    def __init__(self, window: pygame.Surface) -> None:
        ...
        self.cells = [[SudokuCell(self.board[i][j], i, j)
    ...
    def draw_board(self) -> None:
        ...
        for i in range(10):
            if i % 3 == 0:
                pygame.draw.line(self.window, GRIDLINE_COLOR, (0, CELL_SIDE_LENGTH * i),
                                 (BOARD_WIDTH, CELL_SIDE_LENGTH * i), THICK_LINE_WIDTH)
            else:
                pygame.draw.line(self.window, GRIDLINE_COLOR, (0, CELL_SIDE_LENGTH * i),
                                 (BOARD_WIDTH, CELL_SIDE_LENGTH * i), THIN_LINE_WIDTH)

        # draw vertical gridlines
        for i in range(10):
            if i % 3 == 0:
                pygame.draw.line(self.window, GRIDLINE_COLOR, (CELL_SIDE_LENGTH * i, 0),
                                 (CELL_SIDE_LENGTH * i, BOARD_HEIGHT), THICK_LINE_WIDTH)
            else:
                pygame.draw.line(self.window, GRIDLINE_COLOR, (CELL_SIDE_LENGTH * i, 0),
                                 (CELL_SIDE_LENGTH * i, BOARD_HEIGHT), THIN_LINE_WIDTH)
    ...
    def display_footer_message(self, message: str) -> None:
        ...
        self.window.blit(footer_text, (BOARD_WIDTH // 2 - footer_text_width //
                         2, BOARD_HEIGHT + FOOTER_HEIGHT // 2 - footer_text_height // 2))
...
class SudokuCell:
    ...
    def draw_cell(self, window: pygame.Surface) -> None:
        ...
        if self.value:
            ...
            value_text = value_text_font.render(
                str(self.value), True, CELL_FONT_COLOR)

            value_text_width = value_text.get_width()
            value_text_height = value_text.get_height()

            window.blit(value_text, (cell_x + CELL_SIDE_LENGTH // 2 - value_text_width //
                        2, cell_y + CELL_SIDE_LENGTH // 2 - value_text_height // 2))

        # display guess if one exists
        if self.guess:
            guess_font = pygame.font.Font(FONT_FILE_PATH, GUESS_FONT_SIZE)
            guess_text = guess_font.render(
                str(self.guess), True, GUESS_FONT_COLOR)

            window.blit(guess_text, (cell_x + GUESS_FONT_SIZE //
                        2, cell_y + GUESS_FONT_SIZE // 2))

        # if a cell is selected, add a border around it
        if self.is_selected:
            select_border = pygame.Rect(
                cell_x, cell_y, CELL_SIDE_LENGTH, CELL_SIDE_LENGTH)
            pygame.draw.rect(window, SELECT_BORDER_COLOR,
                             select_border, THICK_LINE_WIDTH)
